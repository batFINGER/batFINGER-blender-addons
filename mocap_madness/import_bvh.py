...
class BVH_Node(object):
    __slots__ = (
        ...
        'zero_length', # is this a zero length Node... Add nice handle etc
        )
    ...
    def __repr__(self):
        return ('BVH name:"%s", rest_loc:(%.4f,%.4f,%.4f), rest_tail:(%.4f,%.4f,%.4f)' %
                (self.name,
...
def read_bvh(context, file_path, rotate_mode='XYZ', global_scale=1.0):
    ...
    for bvh_node in bvh_nodes_list:
        ...
        if (bvh_node.rest_tail_local - bvh_node.rest_head_local).length <= 0.001 * global_scale:
            ...
            setattr(bvh_node, "zero_length", True)
            # zero length nodes handle here
...
def load(operator,
    ...
    for f in files:
        operator.count += 1
        mocapfile = os.path.join(directory,f.name)

        t1 = time.time()
        print('\tparsing bvh %r...' % filepath, end="")

        bvh_nodes, bvh_frame_time = read_bvh(context, mocapfile,
        ...
        bvh_name = bpy.path.display_name_from_filepath(mocapfile)

        if target == 'ARMATURE':
            bvh_node_dict2armature(context, bvh_name,
                                   bvh_nodes, bvh_frame_time,
                                   rotate_mode=rotate_mode,
                                   frame_start=frame_start,
                                   IMPORT_LOOP=use_cyclic,
                                   global_matrix=global_matrix,
                                   use_fps_scale=use_fps_scale,
                                   use_frame_step=use_frame_step,
                                   only_rootbone_location=only_rootbone_location,
                                   rig_only=rig_only,
                               )
