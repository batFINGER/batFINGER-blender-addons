...
class BGL_SoundActionFCurveWidget(BGLWidget):
    ...
    def draw_sound_action(self, context, action, x, y, w, h, frames, color=(1,1,1,1)):
        ...
        for fcurve in fcurves:
            ...
            coll = fcurve.sampled_points if len(fcurve.sampled_points) else fcurve.keyframe_points
...
class ScreenActionOperator(bpy.types.Operator):
...
class BGL_Draw_VisualiserPanel(SupportedAreas, Panel):
...
class ScreenAreaAction(bpy.types.Scene):

    def create_area(self, key, index, area):
        a = self.sound_driver_areas.add()
        a.name = key
        a.type = area.type
        a.index = index
        return a
    ...
    def get_area(self, area):
        ...
        return self.sound_driver_areas.get(key)
    ...
    def set_action_to_area(self, area, action):
        ...
        index, key = self.key(area)
        store = self.sound_driver_areas.get(key)
        if store is None:
...
def reg_screen_action_bgl():
    prop_dic = {"loc": IntVectorProperty(size=2, default=(24, 24), min=0),
                ...
                "height": IntProperty(default=0, min=20, max=100, step=1, subtype='PERCENTAGE', description="Height (Percent of View)", update=update_graph),
    ...
    propdic = {"area_index": IntProperty(default=0),
